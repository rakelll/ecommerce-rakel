                    ...
                    <table class="w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="font-semibold text-left">TVA (11%)</th>
                        ...
                        <tbody>
                            @forelse ($cart_items as $item)
                                <tr wire:key='{{ $item['product_id'] }}'>
                                    ...
                                    <td class="py-4">
                                        {{ Number::currency($item['unit_amount'] * 0.11 * $item['quantity']) }}</td>
                                    <td class="py-4">
                                        {{ Number::currency(($item['unit_amount'] * 0.11 + $item['unit_amount']) * $item['quantity']) }}
                                    </td>
                                    <td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-4 text-center">No items in cart!</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="md:w-1/4">
                <div class="p-6 bg-white rounded-lg shadow-md">
                    ...
                    <div class="flex justify-between mb-2">
                        <span>Subtotal</span>
                        <span>{{ Number::currency($subtotal) }}</span> <!-- Use subtotal variable -->
                    </div>
                    <div class="flex justify-between mb-2">
                        <span>Taxes</span>
                        <span>{{ Number::currency($taxes) }}</span> <!-- Use taxes variable -->
                    </div>
                    <div class="flex justify-between mb-2">
                        <span>Shipping</span>
                        <span>$0.00</span>
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        <span class="font-semibold">{{ Number::currency($total) }}</span> <!-- Use total variable -->
                    </div>
                    @if ($cart_items)
